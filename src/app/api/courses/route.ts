...
export async function GET(request: NextRequest) {
  ...
  const result = await hubFetch("/CourseFinder/GetCourseList/", { cookie, body })
  ...
  if (typeof raw === "string") {
    return Response.json(
      { error: "Archers Hub returned an error — your cookie may be expired, or this term has no data yet. Try switching terms or re-pasting your cookie.", detail: raw },
      { status: 401 }
    )
  }
